    ...
    >
      <template #header>
        <b-row>
          ...
          <b-col cols="1" class="text-right">
            <b-button v-if="isStaff" to="organizations/create" variant="success">
      ...
      >
        ...
        <b-col v-if="isStaff" cols="2" class="text-right">
          <b-button size="sm" :to="`/organizations/${item.id}`" variant="warning">
...
export default Vue.extend({
  ...
  async asyncData({ $axios, store }) {
    const { data: items } = await $axios.get('api/corehr/organization/')
    return {
      items,
  ...
  methods: {
    showModal(item: {
      ...
      .then(() => {
        this.$axios.delete(`api/corehr/organization/${item.id}/`)
        .then(() => {
          this.$router.go(0)
        })
      })
