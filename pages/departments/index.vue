    ...
    >
      <template #header>
        <b-row>
          ...
          <b-col cols="1" class="text-right">
            <b-button to="departments/create" variant="success">
      ...
      >
        ...
        <b-col cols="2" class="text-right">
          <b-button size="sm" :to="`/departments/${item.id}`" variant="warning">
...
export default Vue.extend({
  ...
  async asyncData({ $axios }) {
    const { data: items } = await $axios.get('api/corehr/department/')
    return { items }
  },
  methods: {
    showModal(item: {
      ...
      .then(() => {
        this.$axios.delete(`api/corehr/department/${item.id}/`)
        .then(() => {
          this.$router.go()
        })
      })
